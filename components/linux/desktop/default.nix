...
let
  ...
  personalPackages =
    if (config.personalConfig.machineType == "personal")
    then
      with pkgs; [
        mpvScripts.mpris
        playerctl
...
in
{
  ...
  config = lib.mkMerge ([
    (lib.mkIf desktopEnabled (
      trace ''Adding Udev Rules for desktop devices''
        {
    ...
    (lib.mkIf desktopEnabled (
      trace ''Enabling Display Manager''
        {
    ...
    (lib.mkIf desktopEnabled (
      { home-manager.users = desktopUsers; }
    ))
