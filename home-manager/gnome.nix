...
with lib; let
  # Only apply gnome config on Linux
  isLinux = pkgs.stdenv.isLinux;
  
  # Try to get username from home.username if it exists
  hasHomeUsername = config ? home && config.home ? username;
  username = if hasHomeUsername then config.home.username else null;
  
  # Get user config if we have both username and personalConfig
  ...
  # Check if gnome is enabled for this user
  gnomeEnabled = userConfig ? desktop && userConfig.desktop == "gnome";
in {
  config = mkIf (isLinux && gnomeEnabled && options ? dconf) {
